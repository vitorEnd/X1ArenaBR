  ...
  return (
    <>
      ...
      <section className="section section--graphite">
        <div className="page-container"><SectionHeading eyebrow="Quem constrói a Arena" title={<>Três nomes.<br /><span className="title-accent">Uma comunidade.</span></>} description="Os criadores que deram início à Arena X1 Brasil e seguem construindo esta comunidade." /><div className="about-creators-list">{creators.map((creator, index) => <article key={creator.id}><span>0{index + 1}</span><Shield size={22} /><h2>{creator.name}</h2></article>)}</div></div>
      </section>
