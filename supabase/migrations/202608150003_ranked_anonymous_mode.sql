-- Ranked privacy controls: anonymous public identity and private statistics.
...
begin
  ...
  set anonymous_mode = coalesce(p_enabled, false),
      anonymous_number = case
        when coalesce(p_enabled, false) and anonymous_number is null
          then nextval('public.ranked_anonymous_number_seq')
        else anonymous_number
      end
  where id = v_user_id
