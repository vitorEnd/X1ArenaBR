...
begin
  ...
  perform set_config('app.ranked_global_reset', 'on', true);
  ...
  -- O papel service_role usa a proteção safeupdate; WHERE true mantém o
  -- reset explícito sem enfraquecer essa proteção para outras consultas.
  delete from public.ranked_match_reports where true;
  delete from public.ranked_post_match_choices where true;
  delete from public.ranked_result_confirmations where true;
  delete from public.ranked_match_acceptances where true;
  delete from public.ranked_active_match_players where true;
  delete from public.ranked_queue_entries where true;
  delete from public.ranked_mmr_ledger where true;
  delete from public.ranked_matches where true;
  delete from public.ranked_notifications where true;
  delete from public.ranked_rate_limits where true;
  ...
  insert into public.support_audit_log (
    ...
    p_support_user_id, 'reset_ranked', 'ranked_system', 'global',
